...
if __name__ == "__main__":
    ...
    for A in A_list:
        print("Matrix: \n{}\n\n".format(A))
        print("calculated by the built-in function eigvals: " + str(np.linalg.eigvals(A)) + "\n\n")

        for accuracy in acc_list:
            print("accuracy: {}\n".format(accuracy))
            n = A.shape[0]
            eigval, eigvect, steps_num = iterations_method(A, np.zeros(n) + 1, accuracy)
            print("found eigval: " + str(eigval))
            print("found eigvect: " + str(eigvect))
            print("steps num = " + str(steps_num) + "\n")
